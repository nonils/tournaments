...
export class CompetitorController implements interfaces.Controller {
    ...
    public async createNewCompetitor(req: Request, res: Response, next: NextFunction) {
        let response = await this._competitorService.SubscribeCompetitor(CompetitorMapper.MapFromCompetitorRequestToCompetitor(CompetitorRequest.fromBodyToCompetitorRequest(req.body)));
        res.send(response);
    }
